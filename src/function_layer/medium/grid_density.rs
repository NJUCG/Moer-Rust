...
impl GridDensityMedium {
    pub fn new(sigma_a: SpectrumRGB, sigma_s: SpectrumRGB, g: f32,
               nx: usize, ny: usize, nz: usize,
               medium2world: Transform, density: Vec<f32>) -> Self {
        ...
        Self { sigma_a, sigma_s, g, nx, ny, nz, medium2world, density, sigma_t, inv_max_density }
    }

    pub fn d(&self, p: Point3<usize>) -> f32 {
        if p.x >= self.nx || p.y >= self.ny || p.z >= self.nz { return 0.0; }
    ...
    pub fn density(&self, p: Point3<f32>) -> f32 {
        ...
        let pi = Point3::new(p_samples.x.floor(), p_samples.y.floor(), p_samples.z.floor());
        let d = p_samples - pi;
        let pi = Point3::new(pi.x as usize, pi.y as usize, pi.z as usize);
        let d00 = lerp(d.x, self.d(pi), self.d(pi + Vector3::new(1, 0, 0)));
        let d10 = lerp(d.x, self.d(pi + Vector3::new(0, 1, 0)), self.d(pi + Vector3::new(1, 1, 0)));
        let d01 = lerp(d.x, self.d(pi + Vector3::new(0, 0, 1)), self.d(pi + Vector3::new(1, 0, 1)));
        let d11 = lerp(d.x, self.d(pi + Vector3::new(0, 1, 1)), self.d(pi + Vector3::new(1, 1, 1)));
...
impl Medium for GridDensityMedium {
    fn tr(&self, ray: &Ray, sampler: Rc<RefCell<dyn Sampler>>) -> SpectrumRGB {
        ...
        (0..256).map(|_| sampler.borrow_mut().next_1d())
