...
impl Disk {
    pub fn from_json(json: &Value) -> Self {
        ...
        let p1 = shape.transform.to_world_vec(&V3f::new(-radius, -radius, 0.0));
        let p2 = shape.transform.to_world_vec(&V3f::new(radius, radius, 0.0));
        println!("{:?}", shape.transform);
        shape.bounds3 = Bounds3::new(p1, p2);
        Self {
            shape,
            radius,
            inner_radius,
